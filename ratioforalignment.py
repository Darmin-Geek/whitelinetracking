...
def findDistance(centerArray, xRight, xLeft):
    print("x's",xRight,xLeft)
    xLineCenter = .5 * (xRight + xLeft)
    print("xlinecenter", xLineCenter)
    print(centerArray)
    distance = (2 * (xLineCenter - centerArray[0]))/(xRight - xLeft)
    #if negative, line is on left side
    return distance


def main():
    originalImage = cv2.imread("/Users/hankinhomecomputer/Documents/PhotoshopRectangle (2).png")
    ...
    print("distance:",findDistance(findCenter(originalImage),x2,xT))
